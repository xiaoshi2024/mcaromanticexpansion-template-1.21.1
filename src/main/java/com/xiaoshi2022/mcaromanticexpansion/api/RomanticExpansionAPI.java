 ...
 * MinecraftForge.EVENT_BUS.addListener(this::onRomanticEvent);
 *
 * // 5. 检查玩家婚姻状态
...
public final class RomanticExpansionAPI {
    ...
    public static IAffectionManager getAffectionManager() {
        if (affectionManagerImpl == null) {
            affectionManagerImpl = new IAffectionManager() {
                ...
                @Override
                public void handleInteraction(InteractionType type, Player player, Player target) {
     ...
     * <p>
     * 会同时触发 {@link RomanticEventTriggeredEvent} 到 Forge 事件总线。
     *
     * @param player   玩家A
     * @param partner  玩家B
    ...
    private static void sendPacket(ServerPlayer target, ServerPlayer proposer) {
        OpenProposalGUIPacket packet = new OpenProposalGUIPacket(
                proposer.getUUID(), proposer.getName().getString()
        );
        // Forge 网络发送 - 使用 ModNetwork

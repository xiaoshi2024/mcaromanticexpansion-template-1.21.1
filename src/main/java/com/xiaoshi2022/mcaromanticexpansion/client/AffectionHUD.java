...
public class AffectionHUD {
    ...
    private static void onRenderHUD(RenderGuiLayerEvent.Post event) {
        ...
        // 使用 GuiGraphicsExtractor 替代 GuiGraphics
        ...
        guiGraphics.text(minecraft.font, title, baseX + (HUD_WIDTH - titleWidth) / 2, baseY, 0xFFFFFF);

        // 分隔线 - 使用 horizontalLine
        int lineY = baseY + 12;
        guiGraphics.horizontalLine(baseX + 10, baseX + HUD_WIDTH - 10, lineY, 0xAAFFFFFF);
        ...
        guiGraphics.text(minecraft.font, valueText, baseX + (HUD_WIDTH - valueWidth) / 2, barY + barHeight + 4, 0xFFFFFF);
        ...
        if (levelKey != null) {
            ...
            guiGraphics.text(minecraft.font, levelComponent, baseX + (HUD_WIDTH - levelWidth) / 2, barY + barHeight + 18, 0x888888);
        }
        ...
        if (timeSinceChange < CHANGE_DISPLAY_DURATION && lastAffection != affection) {
            ...
            guiGraphics.text(minecraft.font, changeComponent, baseX + HUD_WIDTH / 2 + 30, barY + barHeight + 4, changeColor);
        }
        ...
        if (showDebugInfo) {
            ...
            guiGraphics.text(minecraft.font, debugComponent, 10, 10, 0x888888);
        }
    }

    // 修改 drawHeartIcon 方法使用 GuiGraphicsExtractor
    private static void drawHeartIcon(GuiGraphicsExtractor guiGraphics, int x, int y, int color) {
        int size = 6;
        int halfSize = size / 2;
